...
In our projects, we often video-record children (and their parents) during the experiment. Before we code their behaviour, we have to process the videos (e.g., concatenate, synchronise, etc) because our video cameras store these recrdings as short clips. 
   ...
   - [rename-merge-videos.py](https://github.com/smy1/swlab/blob/main/script/rename-merge-videos.py) This script is useful if we need to rename the video files before merging. It adds a chunk of "check-and-rename" code to the _merge-videos_ script. This is necessary because when syncing the videos, I rely on the name of the videos, which contains the minute and second at which the video recording was taken. In some cases, the recording starts at the 59th minute (e.g., 09:59am) and then continue to the next hour (e.g., 10:00am, 10:01am, etc), which will mess up the sequence of the merging (because 00 comes before 59). In the _merge-videos_ script, we have to check and rename the files manually. With the check-and-rename chunk, Python will change the "00" in the file name to "60" so that the "59" recording is placed before the originally-named-as-"00" recording. 
...
Once we have single, merged videos from each camera, we can sync and juxtapose these videos so that we see the recordings of every participant from different angles. 
   ...
   - [crop-sync-2videos.py](https://github.com/smy1/swlab/blob/main/script/crop-sync-2videos.py) This script adds an additional chunk of code to the _sbr-sync-2videos_ script to crop one of the videos before syncing both of them. It also has an additional line to handle exceptions, which usually happens due to the video duration being shorter than the number written in the script. 
...
### Helpful resources
I relied heavily on the links below when writing these codes. These links use MoviePy v1.0. For details regarding MoviePy v2.0, see the [main page of MoviePy](https://zulko.github.io/moviepy/). 
