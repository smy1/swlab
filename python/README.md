...
Most of my previous scripts have now been compiled into a module called [editvid.py](./editvid.py) (see [previous scripts](./obsolete/) that have _not_ been compiled). This module should be downloaded and stored in the same folder as where we will be running our code. Briefly, the functions in this module edit videos in bulk, allowing the video-editing task to be automatised. Arguments can be provided to the functions in two ways, either by loading an excel file or manually entering the arguments. The [examples.py](./examples.py) script shows how to manually pass arguments to these functions while the [examples below](#examples) explain how these arguments can be given through an excel file.
